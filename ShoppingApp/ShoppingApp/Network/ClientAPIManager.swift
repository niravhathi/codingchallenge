        ...
        if NetworkManager.isConnectedToNetwork() {
            ...
            task = urlSession.dataTask(with: components.url!) { (data, response, error) in
                ...
                guard let data = data
                    ...
                    return completionHandler(.failure(.parsingError(error: error?.localizedDescription ?? Constant.noNewItems)))
                }
                do {
            ...
            return completionHandler(.failure(.networkError(error:  Constant.noInternet, response: nil)))
        }
    }
}
