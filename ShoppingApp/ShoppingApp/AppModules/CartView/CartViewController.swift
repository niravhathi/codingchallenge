...
extension CartViewController: CartPresenterToViewProtocol {
    func responseSuccessRemoveCartToView() {
        self.showAlertMessage(title: "Success", message: Constant.removeCartSuccess)
        cartPresenter?.requestCartDataToPresenter()
    }
...
extension CartViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let iCell =  tableView.dequeueReusableCell(withIdentifier: Constant.cartTableViewCell, for: indexPath) as! CartTableViewCell
